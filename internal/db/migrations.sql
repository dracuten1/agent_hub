...
-- fn_task_completed: update phase counts when task completes
-- Phase advancement is handled by Go code (CompleteTask → advanceWorkflow)
-- to support complex phase types (multi, per_dev, decision, gate)
...
DECLARE
    ...
    v_phase_id TEXT;
BEGIN
    SELECT wtm.phase_id INTO v_phase_id
      FROM workflow_task_map wtm
     WHERE wtm.task_id = NEW.id;

    IF v_phase_id IS NOT NULL THEN
        UPDATE workflow_phases SET completed_tasks = completed_tasks + 1 WHERE id = v_phase_id;
    END IF;
